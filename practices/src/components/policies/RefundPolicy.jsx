...
const RefundPolicy = () => {
  ...
  const { refundPolicyAccepted,setRefundPolicyAccepted} = useContext(myContext);
  ...
  function handleAcceptance(e){
    ...
    setRefundPolicyAccepted(isChecked);
}

  const policy = usePolicies({type:"refund_policy"});
  if (!policy) {
    return (
  ...
  return (
    <>
      ...
      <div className="w-full flex gap-3 font-bold mt-4 p-3" style={{boxShadow:"3px 3px 15px 3px rgba(0,0,0,0.2)"}}>
        <Inputs type="checkbox" name="returnPolicy" className="accent-slate-900 w-5 cursor-pointer" value="accepted return policy" checked={refundPolicyAccepted} onChange={handleAcceptance}/>
